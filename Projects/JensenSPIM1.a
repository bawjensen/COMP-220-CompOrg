...
after_inLoop:
	...
	move	$t2, $t0		# Set the high end of the array
...
after_loop1:
	...
	mflo	$s3			# Overwrite sum with mean

	# ------------------------------------------------------------------------

	li	$t0, 0			# Reset the index
	li	$t4, 0			# Initialize the summation value with 0
...
outLoop: # Prints out the bar graph based on counts array
	...
	li	$v0, 1
	move	$a0, $t0
	div 	$a0, $t1		# lo = a0 / t1 (aka: memory location / memory size)
	mflo	$a0			# a0 = lo
	addi	$a0, $a0, 1 		# Index for 5's count is 4, so add 1
